...
unique(d$DoNotUse)
filtered_d = d %>% 
  filter(is.na(DoNotUse) | DoNotUse != 1) # exclude subjects that are marked as "DoNotUse"
```
...
performance_hyp_d = filtered_d %>% 
  select(-contains("Game")) # remove the columns containing "Game" in the name
head(performance_hyp_d)
```
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarize(
    ...
    SEM = sd(ImprovementScore, na.rm=T) / sqrt(n())
            )
...
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
  geom_bar(position="dodge", stat="identity") + 
  geom_errorbar(aes(ymin = MeanImprovementScore - SEM, 
                    ymax = MeanImprovementScore + SEM),
